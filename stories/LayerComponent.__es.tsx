...
// import { updateTaskState } from '../src/store/store';
...
export const LayerSelectorStory = (): React.ReactElement => {
  ...
  const r = React.useRef<UseLayerFromServiceInterface>();

  React.useEffect(() => {
    console.log('new effect');
    r.current = useLayerFromService(wmsService);
  }, [wmsService]);

  console.log('render', count);

  if (!r.current) return null;
  const { layer, availableLayers, setLayer } = r.current;
